...
class AudioAppState extends State<AudioApp> {
  ...
  void initAudioPlayer() {
    ...
    audioPlayer.setPositionHandler((d) => setState(() {
      position = d;
    }));
  ...
  Future play() async {
    ...
    if (result == 1) {
      setState(() {
         print('AudioState.play... PlayerState.playing');
  ...
  Future _loadFile() async {
    final bytes = await _loadFileBytes(kUrl, 
      onError: (Exception exception) => print('_loadFile => exception ${exception.toString()}'));
